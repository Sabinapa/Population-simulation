...
class Clover(Entity):
    ...
    # Označi radič kot pojeden
    def eat(self):
        self.eaten = True

    # Risanje: vijolična pika za nepojeden radič, temno vijolična za pojedenega
...
# Osnovna žival – skupno obnašanje za zajce in lisice
class Animal(Entity):
    ...
    def __init__(self, x, y, gender: str, cfg,
        ...
        self.repro_cooldown = random.uniform(20.0, 40.0) # čas do ponovne možnosti razmnoževanja med paritvama
    ...
    def _base_update(self, dt, terrain):
        ...
        # Reprodukcijski nagon se polni le ko ni na hladnem (repro_cooldown) in ni kritično žejen/lak
        ...
        # 3 vzroki smrti: dehidracija, lakota in starost
    ...
    # Pemakne se proti cilju
    def _move_towards(self, tx, ty, dt, terrain):

        dx, dy = tx - self.x, ty - self.y
        if math.hypot(dx, dy) < 1.0: # že blizu cilja, ni potrebe po gibanju
            return
    ...
    # Drsi ob ovirah če je pot blokirana, poskuša se premakniti v pravokotne smeri kot zadnja možnost.
    def _slide_move(self, nx, ny, step, terrain):
        ...
        # Najprej poskusi premik v želeni smeri
        ...
        # Premik v horizontalni smeri - drsenje ob navpičnih steni
        if terrain.is_passable(new_x, self.y):
            self.x   = new_x
            did_move = True
        # Premik v vertikalni smeri - drsenje ob vodoravni steni
    ...
    def _wander(self, dt, terrain):
        ...
        # Če je čas za novo smer, izberi naključno smer in resetiraj timer.
        ...
        if terrain.is_passable(new_x, new_y): # prosto v želeni smeri
            self.x, self.y = new_x, new_y
        ...
        else: # oboje blokirano, izberi novo naključno smer
            angle              = random.uniform(0, 2 * math.pi)
            self._dir_x        = math.cos(angle)
            self._dir_y        = math.sin(angle)
    ...
    # Risanje: krog za telo, obroba, krog zaznavnega radija in indikatorji stanja
...
class Rabbit(Animal):
    ...
    def update(self, dt, terrain, foxes, rabbits, clovers, new_rabbits: list):
        ...
        if (self.repro >= self.repro_drive
            ...
            partner = self._find_partner(rabbits) # poišči najboljšega kandidata za parjenje
            if partner:
                # Če je partner blizu, se razmnoži; sicer se premakni proti partnerju.
        ...
        # Prioriteta 2: žeja
        if self.thirst > 0.45:
            water = terrain.nearest_water(self.x, self.y, 320) # poišči najbližjo vodo znotraj zaznavnega radija

            #  Če je voda blizu, se premakni proti njej in pij; sicer nadaljuj z naslednjo prioriteto.
            if water:
        ...
        if self.hunger > 0.40:
            target = self._find_food(clovers) # poišči najbližji nepojeden radič znotraj zaznavnega radija

            # Če je radič blizu, ga pojej; sicer se premakni proti njemu. Če ni nobenega radiča, nadaljuj z tavanje.
            if target:
    ...
    # Poišče najbližji nepojeden radič znotraj zaznavnega radija.
    def _find_food(self, clovers):
        best, best_score = None, self.sense_radius

        # Preišče vse radiče in izbere tistega, ki ni pojeden in je najbližje
        # (manjša razdalja = boljši rezultat).
        for clover in clovers:
            if clover.eaten:
                continue
            distance = _dist(self.x, self.y, clover.x, clover.y) # razdalja do radiča
            if distance < best_score: # bližje = boljše
                best_score, best = distance, clover
        return best

    # # Vrne najboljšega parnega kandidata
    def _find_partner(self, rabbits):
        best, best_score = None, self.sense_radius

        # Preišče vse zajce in izbere tistega, ki je:
        # nasprotnega spola,
        # ni na hladnem (repro_cooldown) in
        # ni mrtv,
        # ter je najbližje (manjša razdalja / večja velikost = boljši rezultat).
    ...
    def _reproduce(self, partner, rabbits, new_rabbits):
        ...
        # Prepreči prekomerno razmnoževanje
        if len(rabbits) + len(new_rabbits) >= cfg.max_rabbits:
            return

        # Ustvari 1–3 potomce z naključnimi lastnostmi, ki so podedovane od staršev z variacijo in mutacijo.
    ...
    def _flee(self, foxes, dt, terrain) -> bool:
        ...
        # Poišče vse lisice znotraj zaznavnega radija.
        ...
        self._slide_move(ndx, ndy, step, terrain) # premik v smeri bega
        self._clamp(terrain) # ohranja znotraj meja mape
        return True
...
class Fox(Animal):
    ...
    def update(self, dt, terrain, foxes, rabbits, new_foxes: list):
        ...
        if (self.repro >= self.repro_drive
            ...
            # Poišči najboljšega kandidata za parjenje (nasprotnega spola, ni na hladnem, ni mrtv, bližje in večje je boljše).
            ...
            if partner:
                if _dist(self.x, self.y, partner.x, partner.y) < FOX_MATE_RADIUS:
                    ...
                    return
                self._move_towards(partner.x, partner.y, dt, terrain) # premik proti partnerju
                return
        ...
        # Prioriteta 3: lov
        if self.hunger > 0.30:
            # Poišči najboljšo tarčo za lov (najlažje ujeti zajce so tisti, ki so bližje, večji in počasnejši).
            ...
            if prey:
                ...
                else: # premik proti tarči
                    self._move_towards(prey.x, prey.y, dt, terrain)
                return

        self._wander(dt, terrain)

    # Poišče najboljšo tarčo za lov (najlažje ujeti zajce so tisti, ki so bliže, večji in počasnejši).
    def _find_best_prey(self, rabbits):
        best, best_score = None, float("inf")

        # Preišče vse zajce
        for rabbit in rabbits:
            if rabbit.dead: # izključi mrtve zajce
                continue

            distance = _dist(self.x, self.y, rabbit.x, rabbit.y)

            if distance > self.sense_radius: # izključi zajce zunaj zaznavnega radija
                continue

            speed_factor = self.speed / max(rabbit.speed, 1.0) # zajci, ki so počasnejši od lisice, so lažji za ujeti
            score        = distance / (rabbit.size * speed_factor + 1e-9) # manjši rezultat = bližje, večje in počasnejše tarče so boljše

            # Manjši rezultat = bližje, večje in počasnejše tarče so boljše.
            if score < best_score:
                best_score, best = score, rabbit
        return best
    ...
    def _find_partner(self, foxes):
        ...
        # Preišče vse lisice in izbere tistega, ki je:
        # nasprotnega spola,
        # ni na hladnem (repro_cooldown) in
        # ni mrtv,
        # ter je bližje in večje (manjša razdalja / večja velikost = boljši rezultat).
        for fox in foxes:
    ...
    def _reproduce(self, partner, foxes, new_foxes):
        ...
        # Ustvari 1–2 potomca z naključnimi lastnostmi, ki so podedovane od staršev z variacijo in mutacijo.
